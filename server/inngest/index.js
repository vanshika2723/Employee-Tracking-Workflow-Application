...
export const inngest = new Inngest({ id: "employee-workflow" });
...
const autoCheckOut = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    message:"You have completed 9 working hours. Please logout.",
    ...
    if (leaveApplication?.status === "PENDING") {
      ...
      await sendEmail({
        ...
        body: `<div style="max-width:600px";>
            ...
            <p style="font-size:16px;">Please make sure to to take action on this leave application.</p>
...
const attendanceRemainderCron = inngest.createFunction(
  ...
  async ({ step }) => {
    ...
    await step.run(
    "create-attendance-warning-notifications",
    async()=>{
    ...
    if (absentEmployees.length > 0) {
      await step.run("send-remainder-emails", async () => {
        ...
        return { emailsSend: absentEmployees.length };
      });
    }
...
// console.log("Inngest Functions Loaded:", functions.length);
